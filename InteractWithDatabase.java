...
public class InteractWithDatabase {
    ...
    private static void runConsole(Database database)
    {
        ...
        while (line != null && !line.equals("exit")) {
            ...
            else if (parts[0].equals("RankTeams"))
            {
                database.standings();
                printDescription();
            }
            ...
            else if (parts[0].equals("playerStatsInd"))
            {
                    if (parts.length >= 2) {
                        database.playerStatsSort(arg);
                    }
                    else
    ...
    private static void printTeam() {
        ...
        System.out.println(getPrintHelpCommands("MostPtsAtLocation <location>", "- Enter first few letters of location or \"City, State\" format.\n" +
        ...
        System.out.println(getPrintHelpCommands("RankTeams", "- League Standings."));
        System.out.println("==========================================================================================================================================================");
    }
    ...
    private static void printPlayer() {
        ...
        System.out.println(getPrintHelpCommands("playerStatsInd <name>", "- Provide either exact player names or some letters in the name.\n" +
                getPrintHelpCommands("","  Important because NBA Analysts are always looking up players stats.")));
        System.out.println("===========================================================================================================================================================");
    }
...
class Database {
    ...
    public void mostEfficientPlayers()
    {
        try {
            String sql = "SET ARITHABORT OFF\n" +
                    ...
                    "SELECT top 25 playersStats.player_name, playerStatsPerTeam.team_abbrev,\n" +
                    "(playersStats.total_pts + playersStats.tot_rb + \n" +
                    "playersStats.assists + playersStats.steals + \n" +
                    "playersStats.blocks - (playersStats.fg_attempts-playersStats.fg_made) - \n" +
                    "(playersStats.ft_attempts - playersStats.ft_made) - playersStats.turnovers)\n" +
                    "/playersStats.games_played as EFF FROM playersStats\n" +
                    "join playerStatsPerTeam on playerStatsPerTeam.player_id = playersStats.player_id\n" +
                    "ORDER by EFF DESC;";
    ...
    public void teamsWithMostPointsPerPossession() {
        try {
            String sql = "SELECT teams.team_name, CAST(avg(gameTeamPerformance.team_score/gameDetails.team_pace) as DECIMAL (12,3)) as \"Points Per Possession\" from teams\n" +
    ...
    {
        try {
            String sql = "SELECT team_abbrev as \"Team Name\", COUNT(team_abbrev) as Wins from gameDetails\n" +
                    "JOIN  gameTeamPerformance on gameDetails.game_id = gameTeamPerformance.game_id\n" +
    ...
    {
        try {
            String sql = "-- All the players who have played all their team's games\n" +
                    "select playersStats.player_id, playersStats.player_name, outerPlayerStatsPerTeam.team_abbrev, playersStats.games_played from playersStats \n" +
                    ...
                    "        where playerStatsPerTeam.games_played = 82 and not exists (\n" +
    ...
    {
        try {
            String sql = "SELECT teams.team_abbrev, teams.Wins, teams.loses, teams.off_rtg, teams.dff_rtg from teams\n" +
                    "order by wins desc;";
            ...
            while (resultSet.next()) {
                columns[0] = resultSet.getString("team_abbrev");
                columns[1] = resultSet.getString("Wins");
                columns[2] = resultSet.getString("loses");
                columns[3] = resultSet.getString("off_rtg");
                columns[4] = resultSet.getString("dff_rtg");
                output += getOutputTwo(numColumns, columns) + "\n";
            }
            output += "-----------------------------------------------------------------------\n";
            System.out.println(output);
    ...
    public void playerStatsSort(String a)
    {
        try {
            String sql = "SET ARITHABORT OFF\n" +
                    "select playersStats.player_name, playersStats.age, " +
                    "CAST(CAST(playersStats.total_pts AS float)/CAST(playersStats.games_played AS float) as decimal(18,2)) as pts,  " +
                    "CAST(CAST(playersStats.tot_rb AS float)/CAST(playersStats.games_played AS float) as decimal(18,2)) as rb, " +
                    "CAST(CAST(playersStats.assists AS float)/CAST(playersStats.games_played AS float) as decimal(18,2)) as ast, " +
                    "CAST(CAST(playersStats.steals AS float)/CAST(playersStats.games_played AS float) as decimal(18,2)) as stl, " +
                    "CAST(CAST(playersStats.blocks AS float)/CAST(playersStats.games_played AS float) as decimal(18,2)) as blk, " +
                    "CAST(\"fg%\"*100 as decimal(18,2)) as \"fg%\", CAST(\"3p%\"*100 as decimal(18,2)) as \"3p%\", " +
                    "CAST(CAST(playersStats.turnovers AS float)/CAST(playersStats.games_played AS float) as decimal(18,2)) as trn, " +
                    "CAST(CAST(playersStats.fouls AS float)/CAST(playersStats.games_played AS float) as decimal(18,2)) as f  from playersStats\n" +
                    "where playersStats.games_played != 0 and playersStats.player_name like (?)\n" +
                    "order by pts DESC; ";
            ...
            columns[7] = "FG%%";
            columns[8] = "3pt%";
            columns[9] = "Turn pr gm";
            columns[10] = "fls pr gm";
            ...
            while (resultSet.next())
            {
                ...
                columns[1] = resultSet.getString("age");
                columns[2] = resultSet.getString("pts");
                columns[3] = resultSet.getString("rb");
                columns[4] = resultSet.getString("ast");
                columns[5] = resultSet.getString("stl");
                columns[6] = resultSet.getString("blk");
                columns[7] = resultSet.getString("fg%");
                columns[8] = resultSet.getString("3p%");
                columns[9] = resultSet.getString("trn");
                columns[10] = resultSet.getString("f");
